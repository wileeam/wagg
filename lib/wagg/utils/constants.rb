...
module Wagg
  module Utils
    module Constants
      ...
      NEWS_URL = SITE_URL + '/story/%{url_id}'
      ...
      VOTE_RE = /(?<author>.+):\s+(?<timestamp>((\d{1,2}(\/|-)\d{1,2})(-\d{4})?)?(\s|-)?\d{1,2}:\d{1,2}(:\d{1,2})?(\s[A-Z]+)?)(\s(?:valor|karma):[[:space:]]+(?<weight>-?\d+))?/
      # News vote rates
      VOTE_NEWS             =  0
      VOTE_COMMENT          =  1
      VOTE_COMMENT_LIFETIME = 30*24*60*60 # 30 days
      VOTE_COMMENT_UPRATE   =  0
      VOTE_COMMENT_DOWNRATE = -1
      VOTE_NEWS_UPRATE      =  0
