...
module Wagg
  module Crawler
    ...
    class News < NewsSummary
      ...
      class << self
        def parse(id_extended)
          news = News.new(id_extended)
      ...
      def parse_log
        log_uri = ::Wagg::Constants::News::LOG_URL % {id_extended:@id_extended}
      ...
      def parse_status_events(events_table_items)
        ...
        events_table_items.each do |event_item|
          ...
          event_author_matched = event_author_item.match(/\A\/user\/(?<author>.+)\z/)
